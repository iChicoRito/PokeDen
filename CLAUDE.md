...
Husky `pre-commit` runs `lint-staged` (biome autofix on staged JS/TS). A biome error blocks the commit.
...
The design system uses a single theme — the "moss" tweakcn theme — defined by the `:root` / `.dark` CSS-variable blocks in `src/app/globals.css` (Tailwind v4, CSS-first). There are no alternate theme presets; theme mode (light/dark/system) is handled by the preferences system below.
