...
def generate_module(entries: list[dict]) -> str:
    ...
    lines.append("")
    # Source provenance: several .aseprite files are cross-named relative to the art
    # they contain (pixel-verified 2026-09-06). This table records which source file
    # each exported sheet actually came from so the mapping is discoverable.
    lines.append("// Source provenance (sheet -> .aseprite source; several sources are cross-named):")
    for entry in entries:
        source_stem = entry["sourceStem"]
        note = f" // cross-named: holds {entry['displayName']} art" if source_stem != entry["species"] else ""
        lines.append(f"//   {entry['species']}-sheet.png <- {entry['folder']}/{source_stem}.aseprite{note}")
    lines.append("")
